    ...
    for (size_t i = 0; i < data.size(); i++) {
        if(alpha(i) > 1e-5 && alpha(i) < C - 1e-5) {
            ...
            for (size_t j = 0; j < data.size(); j++) {
                b_i += alpha(j) * data[j].y * K(i, j);
            }
            b_i = data[i].y - b_i;
            b_vals.push_back(b_i);
        }
    }
    if(b_vals.empty())
        return 0.0;
...
double calculateAccuracy(const std::vector<DataPoint>& train_data,
    ...
    for (size_t i = 0; i < N_test; ++i) {
        ...
        for (size_t j = 0; j < N_train; ++j) {
            if (alpha(j) > 1e-6) {
                decision_value += alpha(j) * train_data[j].y * polyKernel(train_data[j].x, test_data[i].x);
            }
        }
        decision_value += bias;
...
int main(int argc, char* argv[]) {
    ...
    while ((opt = getopt(argc, argv, "r:s:")) != -1) {
        switch (opt) {
            ...
            case 's':
                ...
                break;
            default:
                std::cerr << "Usage: " << argv[0] << " [-r num_rows] [-s test_size]" << std::endl;
                return 1;
        }
    }
    ...
    try {
        ...
        std::string output_filename = "result_seq/" + std::to_string(num_rows) + "_" + std::to_string(test_size);
        ...
        if (output_file.is_open()) {
            ...
            std::cout << "Sequential results for " << num_rows << " rows, test size " << test_size << ":" << std::endl;
